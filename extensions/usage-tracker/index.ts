...
function parseResetCountdownMs(resetDescription: string | null): number | null {
  ...
  const matches = [
    ...normalized.matchAll(
      /(\d+(?:\.\d+)?)\s*(weeks?|w|days?|d|hours?|hrs?|hr|h|minutes?|mins?|min|m)\b/g,
    ),
  ];
  if (matches.length === 0) {
    return null;
...
function computeWindowPace(window: RateWindow): WindowPace | null {
  ...
  if (
    totalWindowMs <= 0 ||
    resetCountdownMs <= 0 ||
    resetCountdownMs > totalWindowMs
  ) {
    return null;
  }
...
function upsertWindow(
  ...
  if (existing) {
    ...
    existing.resetDescription =
      nextWindow.resetDescription ?? existing.resetDescription;
...
async function getOAuthModule(): Promise<
  typeof import("@mariozechner/pi-ai/oauth") | null
> {
  if (oauthModule) {
    return oauthModule;
  }
...
async function probeAnthropicDirect(
  token: string,
  ...
  try {
    if (isAnthropicOAuthToken(token)) {
      const response = await fetch(
        `${PROVIDER_API_BASE.anthropic}${ANTHROPIC_OAUTH_USAGE_PATH}`,
        {
          method: "GET",
          headers: {
      ...
      if (response.status === 401) {
        result.error =
          "Anthropic auth token expired \u2014 re-authenticate in pi settings.";
        return result;
      }
      ...
      if (result.windows.length === 0) {
        result.note =
          "Anthropic OAuth usage response did not include window data.";
      }
      return result;
    }
    ...
    const response = await fetch(
      `${PROVIDER_API_BASE.anthropic}/v1/messages/count_tokens`,
      {
        method: "POST",
        headers,
        body: JSON.stringify({
    ...
    const reqLimit = Number.parseInt(
      response.headers.get("anthropic-ratelimit-requests-limit") ?? "",
      10,
    );
    const reqRemaining = Number.parseInt(
    ...
    const tokLimit = Number.parseInt(
      response.headers.get("anthropic-ratelimit-tokens-limit") ?? "",
      10,
    );
    const tokRemaining = Number.parseInt(
    ...
    if (
      Number.isFinite(reqLimit) &&
      Number.isFinite(reqRemaining) &&
      reqLimit > 0
    ) {
    ...
    if (
      Number.isFinite(tokLimit) &&
      Number.isFinite(tokRemaining) &&
      tokLimit > 0
    ) {
...
function hydrateOpenAIFromJwt(
  ...
  const auth = jwt["https://api.openai.com/auth"] as
    ...
    | undefined;
  const planType =
    typeof auth?.chatgpt_plan_type === "string" ? auth.chatgpt_plan_type : null;
  if (planType) {
...
function maybeAddOpenAIWhamWindow(
  ...
  const roundedWindowSeconds =
    windowSeconds !== null && windowSeconds > 0
      ? Math.round(windowSeconds)
      : null;
  const labelSuffix = roundedWindowSeconds
    ? windowLabelFromSeconds(roundedWindowSeconds)
    : windowLabel;
  ...
  const resetFromTimestamp =
    resetAtSeconds !== null
      ? countdownFromSeconds(resetAtSeconds - Date.now() / 1000)
      : null;
  ...
  upsertWindow(result.windows, {
    ...
    windowMinutes: roundedWindowSeconds
      ? Math.max(1, Math.round(roundedWindowSeconds / 60))
      : null,
  });
}

function maybeAddOpenAIWhamRateLimitGroup(
  result: ProviderRateLimits,
  groupLabel: string,
  group: unknown,
): void {
  ...
  maybeAddOpenAIWhamWindow(
    result,
    groupLabel,
    "secondary",
    typed.secondary_window,
  );
}
...
async function probeOpenAIDirect(token: string): Promise<ProviderRateLimits> {
  ...
  try {
    ...
    if (response.status === 401) {
      result.error =
        "OpenAI auth token expired — re-authenticate in pi settings.";
      return result;
    }
    if (response.status === 429) {
      const retryAfter = Number.parseInt(
    ...
    maybeAddOpenAIWhamRateLimitGroup(
      result,
      "Code Review",
    ...
    if (result.windows.length === 0) {
      result.note = appendNote(
        result.note,
        "OpenAI usage response did not include window data.",
      );
    }
  } catch (e) {
    if (e instanceof Error && e.name === "TimeoutError") {
...
async function hydrateGoogleAccount(
  result: ProviderRateLimits,
  token: string,
): Promise<void> {
  if (result.account) {
...
async function probeGoogleDirect(
  token: string,
  authEntry?: PiAuthEntry,
): Promise<ProviderRateLimits> {
  const result: ProviderRateLimits = {
    provider: "google",
    windows: [],
  ...
  try {
    const body: Record<string, unknown> = {
      ...
      ...(authEntry?.projectId
        ? { cloudaicompanionProject: authEntry.projectId }
        : {}),
    };

    const response = await fetch(
      `${PROVIDER_API_BASE.google}/v1internal:loadCodeAssist`,
      {
        method: "POST",
    ...
    if (response.status === 401) {
      result.error =
        "Google auth token expired — re-authenticate in pi settings.";
      return result;
    }
    if (!response.ok) {
    ...
    const tierName =
      typeof currentTier?.name === "string" ? currentTier.name : null;
    const tierDescription =
      typeof currentTier?.description === "string"
    ...
    if (tierDescription?.toLowerCase().includes("unlimited")) {
      ...
      result.note = appendNote(
        result.note,
        "Tier reports unlimited coding assistant capacity.",
      );
    }
    ...
    const rateLimit = parseFiniteNumber(
      response.headers.get("x-ratelimit-limit"),
    );
    const rateRemaining = parseFiniteNumber(
    ...
    if (result.windows.length === 0) {
      result.note = appendNote(
        result.note,
        "Rate limit windows are project-scoped and not exposed by this API.",
      );
    }
...
function hasProviderDisplayData(rl: ProviderRateLimits): boolean {
  return (
    rl.windows.length > 0 ||
    rl.credits !== null ||
    Boolean(rl.account || rl.plan || rl.note || rl.error)
  );
}

function shouldPreserveStaleWindows(
...
export default function usageTracker(pi: ExtensionAPI) {
  ...
  function pruneRollingHistory(now = Date.now()): void {
    ...
    for (let i = rollingHistory.length - 1; i >= 0; i--) {
      if (
        !Number.isFinite(rollingHistory[i].timestamp) ||
        rollingHistory[i].timestamp < cutoff
      ) {
        rollingHistory.splice(i, 1);
      }
    }
    if (rollingHistory.length > ROLLING_HISTORY_MAX_POINTS) {
      rollingHistory.splice(
        0,
  ...
  function loadRollingHistory(): void {
    try {
      ...
      const raw = JSON.parse(readFileSync(usageHistoryPath, "utf-8")) as {
        entries?: unknown;
      };
      ...
      for (const item of raw.entries) {
        ...
        if (
          !(Number.isFinite(timestamp) && Number.isFinite(cost)) ||
          cost < 0
        ) {
          continue;
        }
  ...
  function getPace(): { tokensPerMin: number; costPerHour: number } | null {
    ...
    const spanMs =
      turnHistory[turnHistory.length - 1].timestamp - turnHistory[0].timestamp;
    if (spanMs < 10_000) {
      return null;
    }
    let tokenTotal = 0;
  ...
  function checkThresholds(ctx: ExtensionContext): void {
    ...
    for (let i = COST_THRESHOLDS.length - 1; i >= 0; i--) {
      if (cost >= COST_THRESHOLDS[i] && i > lastThresholdIndex) {
        ...
        ctx.ui.notify(
          `Session cost reached ${fmtCost(COST_THRESHOLDS[i])} (now ${fmtCost(cost)})`,
          "warning",
        );
        return;
      }
  ...
  async function probeProvider(
    ...
    if (
      (!force && now - last < PROBE_COOLDOWN_MS) ||
      probeInFlight.has(provider)
    ) {
      return;
    }
    ...
    try {
      ...
      if (shouldPreserveStaleWindows(previous, limits)) {
        limits.windows =
          previous?.windows.map((window) => ({ ...window })) ?? [];
  ...
  function renderRateLimitsPlain(): string {
    ...
    for (const [, rl] of rateLimits) {
      ...
      const windows = [...rl.windows].sort(
        (a, b) => a.percentLeft - b.percentLeft,
      );
      ...
      for (const w of windows) {
        ...
        const reset = w.resetDescription
          ...
          : "";
        lines.push(
          `  ${w.label}: ${bar} ${w.percentLeft}% left (${usedPercent.toFixed(0)}% used)${reset}`,
        );
      ...
      if (most) {
        lines.push(
          `  Most constrained: ${most.label} (${most.percentLeft}% left)`,
        );
      } else if (!rl.error) {
  ...
  function renderRateLimitsRich(theme: {
    ...
    for (const [, rl] of rateLimits) {
      ...
      if (rl.error) {
        lines.push(
          `    ${theme.fg("error", "! Error:")} ${theme.fg("dim", rl.error)}`,
        );
      }
      ...
      for (const w of windows) {
        ...
        const reset = w.resetDescription
          ? theme.fg("dim", ` — resets ${w.resetDescription}`)
          : "";
        lines.push(
          `    ${theme.fg("accent", w.label.padEnd(15))}${bar} ${pct} ${used}${reset}`,
        );
        ...
        if (pace) {
          ...
          const rightText = right
            ? `${theme.fg("dim", " | ")}${theme.fg("dim", right)}`
            : "";
          lines.push(
      ...
      if (most) {
        lines.push(
          `    ${theme.fg("dim", `Most constrained: ${most.label} (${most.percentLeft}% left)`)}`,
        );
      } else if (!rl.error) {
        lines.push(
          `    ${theme.fg("dim", "Windows unavailable from current CLI output")}`,
        );
      }
      if (rl.note) {
      ...
      if (rl.plan) {
        lines.push(
          `    ${theme.fg("accent", "Plan".padEnd(15))}${theme.fg("warning", rl.plan)}`,
        );
      }
      if (rl.account) {
        lines.push(
          `    ${theme.fg("accent", "Account".padEnd(15))}${theme.fg("dim", rl.account)}`,
        );
      }
      if (rl.credits !== null) {
        lines.push(
  ...
  function renderRateLimitsWidget(theme: {
    fg: (c: string, t: string) => string;
  }): string {
    const parts: string[] = [];
    for (const [, rl] of rateLimits) {
      ...
      const most = rl.windows.reduce((a, b) =>
        a.percentLeft < b.percentLeft ? a : b,
      );
      const color = pctColor(most.percentLeft);
      const bar = theme.fg(color, progressBar(most.percentLeft, 8));
      const reset = most.resetDescription
        ? theme.fg("dim", ` (${most.resetDescription})`)
        : "";
      parts.push(
  ...
  function generatePlainReport(ctx: ExtensionContext): string {
    ...
    if (pace) {
      lines.push(
        `Pace: ~${fmtTokens(pace.tokensPerMin)} tokens/min (${fmtCost(pace.costPerHour)}/hour)`,
      );
    }
    if (totals.cacheRead > 0 || totals.cacheWrite > 0) {
      const cacheRatio =
        totals.input > 0 ? (totals.cacheRead / totals.input) * 100 : 0;
      lines.push(
    ...
    if (externalSources.length > 0) {
      const externalTotalCost = externalSources.reduce(
        (sum, source) => sum + source.costTotal,
        0,
      );
      const externalTurns = externalSources.reduce(
        (sum, source) => sum + source.turns,
        0,
      );
      const externalTokens = externalSources.reduce(
        (sum, source) => sum + source.input + source.output,
        0,
      );
      lines.push(
    ...
    if (models.size > 0) {
      ...
      for (const m of sorted) {
        const costShare =
          totals.cost > 0 ? (m.costTotal / totals.cost) * 100 : 0;
        ...
        if (m.cacheRead > 0 || m.cacheWrite > 0) {
          lines.push(
            `    cache: ${fmtTokens(m.cacheRead)} read / ${fmtTokens(m.cacheWrite)} write`,
          );
        }
      }
    }
  ...
  function generateRichReport(
    ...
    if (rlLines.length > 0) {
      ...
      lines.push(
        `  ${theme.fg("dim", "No rate limit data yet — will probe after next turn")}`,
      );
      lines.push("");
    }
    ...
    if (externalSources.length > 0) {
      ...
      if (externalSources.length > 4) {
        lines.push(
          `    ${theme.fg("dim", `+${externalSources.length - 4} more sources`)}`,
        );
      }
    }
    ...
    if (models.size > 0) {
      ...
      for (const m of sorted) {
        ...
        lines.push(
          `  ${theme.fg("accent", "*")} ${theme.fg("accent", m.model)} ${theme.fg("dim", `(${m.provider})`)}`,
        );
        lines.push(
        ...
        lines.push(
          `    ${theme.fg("dim", `avg ${fmtTokens(Math.round(avgTokens))} tok/turn`)}`,
        );
        if (m.cacheRead > 0 || m.cacheWrite > 0) {
          lines.push(
  ...
  function renderWidget(
    ...
    if (totals.turns > 0) {
      ...
      parts.push(
        `${theme.fg("success", fmtTokens(totals.input))}/${theme.fg("warning", fmtTokens(totals.output))}`,
      );
    }

    const externalSources = getExternalSources();
    if (externalSources.length > 0) {
      const externalCost = externalSources.reduce(
        (sum, source) => sum + source.costTotal,
        0,
      );
      parts.push(theme.fg("warning", `Ext:${fmtCost(externalCost)}`));
    }
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    ctx.ui.setWidget("usage-tracker", (tui, theme) => {
      ...
      return {
        ...
        invalidate() {},
        render(width: number) {
          return renderWidget(ctx, theme).map((line) =>
  ...
  pi.registerCommand("usage-toggle", {
    ...
    async handler(_args, ctx) {
      ...
      if (widgetVisible) {
        ...
        ctx.ui.notify(
          "Usage widget hidden. Run /usage-toggle to show.",
          "info",
        );
      }
    },
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "Show provider rate limits (% remaining, reset time) and session usage/cost report.",
    parameters: Type.Object({
      format: Type.Optional(
        Type.Union([Type.Literal("summary"), Type.Literal("detailed")], {
          description:
            "'summary' for rate limits only, 'detailed' for full breakdown. Default: detailed.",
        }),
      ),
    }),
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (format === "summary") {
        ...
        const externalText =
          externalCost > 0 ? ` | external: ${fmtCost(externalCost)}` : "";
