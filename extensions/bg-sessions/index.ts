...
function refreshStatus(): void {
  ...
  const label =
    running.length === 1 ? "1 bg session" : `${running.length} bg sessions`;
...
function spawnBackground(
  ...
  const inv = getPiInvocation([
    ...
    "json",
    "-p",
    "Continue autonomously. The user has switched to another session and will check back later.",
  ]);
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_before_switch", async (event, ctx) => {
    ...
    const action =
      event.reason === "new" ? "create a new session" : "switch sessions";
    ...
    refreshStatus();
    ctx.ui.notify(
      `Session #${bg.id} "${sessionName}" running in background (PID ${bg.pid})`,
      "info",
    );
  });
    ...
    handler: async (args, ctx) => {
      ...
      if (trimmed.startsWith("kill")) {
        ...
        if (rest === "") {
          ...
          refreshStatus();
          ctx.ui.notify(
            `Killed ${running.length} background session(s).`,
            "success",
          );
        } else {
      ...
      const lines = bgSessions.map((bg) => {
        ...
        const icon =
          bg.status === "running" ? "⚡" : bg.status === "done" ? "✓" : "✗";
        const pidStr =
          bg.status === "running"
            ? ` (PID ${bg.pid})`
            : ` (exit ${bg.exitCode ?? "?"})`;
      ...
      ctx.ui.notify(["Background sessions:", ...lines].join("\n"), "info");
    },
  });
}
